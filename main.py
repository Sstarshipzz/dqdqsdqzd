...
async def handle_normal_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Boutons d'annulation
        ...
        if product:
            ...
            CATALOG['stats']['last_updated'] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
            save_catalog(CATALOG)

            # Préparer le message avec la description
            ...
            # Si le produit a une image/vidéo
            if product.get('media_id'):
                ...
                # Si pas de média, afficher uniquement la description
                await query.edit_message_text(
                    text=caption,
                    reply_markup=reply_markup,
                    parse_mode='Markdown'
