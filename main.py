...
async def handle_product_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = await update.message.reply_text("💰 Veuillez entrer le prix du produit :")
...
async def handle_product_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Supprimer le message "Veuillez entrer le prix"
    if context.user_data.get('last_message_id'):
        try:
            await context.bot.delete_message(
                chat_id=update.effective_chat.id,
...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Incrémenter le compteur et ajouter le média
...
async def handle_normal_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ajoutez ces gestionnaires pour la navigation entre les médias
        ...
        try:
            ...
            product = None
            for p in CATALOG.get(category, []):
                if p['name'] == product_name:
                    product = p
                    break
        
            if product and 'media' in product:
                # S'assurer que les médias sont triés par order_index
                sorted_media = sorted(product['media'], key=lambda x: x.get('order_index', 0))
                total_media = len(sorted_media)
                ...
                current_media = sorted_media[current_index]
                media_number = current_index + 1  # Pour l'affichage
            
                caption = f"📱 *{product['name']}*\n\n"
                caption += f"💰 *Prix:*\n{product['price']}\n\n"
                caption += f"📝 *Description:*\n{product['description']}"
                caption += f"\n\n📸 Média {media_number}/{total_media}"
            
                keyboard = []
                if total_media > 1:
                    nav_row = []
                    nav_row.append(InlineKeyboardButton("⬅️ Précédent", 
                                                      callback_data=f"prev_media_{category}_{product_name}"))
                    nav_row.append(InlineKeyboardButton("➡️ Suivant", 
                                                      callback_data=f"next_media_{category}_{product_name}"))
                    keyboard.append(nav_row)
            
                keyboard.append([InlineKeyboardButton("🔙 Retour à la catégorie", 
                                                    callback_data=f"view_{category}")])
            ...
            print(f"Erreur détaillée lors de la navigation des médias: {e}")
            await query.answer("Une erreur est survenue")
