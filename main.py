...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data.get('temp_product_media'):
        context.user_data['temp_product_media'] = []
    
    # Ajouter le nouveau média à la liste
    if update.message.photo:
        context.user_data['temp_product_media'].append({
    ...
    current_count = len(context.user_data['temp_product_media'])
    await update.message.reply_text(
        f"📸 {current_count} média(s) ajouté(s)\n"
...
async def finish_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print("Médias à sauvegarder:", media_list)  # Debug

    if not category:
        return await show_admin_menu(update, context)

    # Créer le nouveau produit avec la liste des médias
    new_product = {
        ...
        'media': media_list  # Sauvegarder la liste complète des médias
    }
        ...
        try:
            ...
            if product:
                ...
                if product.get('media'):
                    ...
                    current_media_index = context.user_data.get('current_media_index', 0)
                    ...
                    if current_media_index < len(media_list) - 1:
                        if len(keyboard[0]) > 0:
                            keyboard[0].append(
                    ...
                    if len(media_list) > 1:
                        caption += f"\n\n📸 Image {current_media_index + 1}/{len(media_list)}"
                
                    current_media = media_list[current_media_index]
                    ...
                    try:
                        ...
                        if current_media['media_type'] == 'photo':
                            ...
                            message = await context.bot.send_video(
                                ...
                                # Réinitialiser l'index des médias
            ...
            print(f"Erreur dans le gestionnaire de produits: {e}")
    ...
    # Ajoutez ces gestionnaires pour la navigation entre les médias
        ...
        try:
            ...
            if product and 'media' in product:
                print("Médias disponibles:", product['media'])  # Debug
                ...
                caption += f"💰 *Prix:*\n{product['price']}\n\n"
                caption += f"📝 *Description:*\n{product['description']}"
                caption += f"\n\n📸 Media {current_index + 1}/{total_media}"
            
                # Préparer les boutons de navigation
                keyboard = []
                if total_media > 1:
                    nav_row = []
                ...
                print(f"Media actuel ({current_index}):", current_media)  # Debug
