...
async def handle_product_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📸 Veuillez envoyer une photo ou une vidéo du produit (ou cliquez sur Ignorer pour ne pas ajouter de média):",
        reply_markup=InlineKeyboardMarkup([
...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Gère l'ajout du média (photo ou vidéo) du produit"""
    category = context.user_data.get('temp_product_category')
    if not category:
        return await show_admin_menu(update, context)
    ...
    if update.message.photo:
        new_product['media_id'] = update.message.photo[-1].file_id
        new_product['media_type'] = 'photo'
    elif update.message.video:
        new_product['media_id'] = update.message.video.file_id
        new_product['media_type'] = 'video'
...
async def handle_normal_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "admin":
        ...
        keyboard = []
        print(f"Produits dans la catégorie {category}: {products}")  # Debug
        ...
        try:
            ...
            if len(parts) < 2:
                print(f"Format invalide du callback_data: {query.data}")
                raise ValueError("Format invalide du callback_data")
            
            category = parts[0]
            product_name = "_".join(parts[1:])  # Gérer les noms de produits qui contiennent des underscores
            
            print(f"Tentative de suppression : catégorie={category}, produit={product_name}")  # Debug
            ...
            if category in CATALOG:
                ...
                print(f"Catégorie {category} non trouvée dans le catalogue")
                await query.message.edit_text(
    ...
    # Boutons d'annulation
        ...
        try:
            ...
            if product:
                ...
                # Si le produit a un média
                if product.get('media_id'):
                    try:
                        # Supprimer le message précédent
                        await query.message.delete()
                    
                        # Envoyer le nouveau message avec média
                        ...
                        context.user_data['last_product_message_id'] = new_message.message_id
                    ...
                    # Si pas de média, modifier simplement le message
                    await query.edit_message_text(
                        text=caption,
                        reply_markup=reply_markup,
                        parse_mode='Markdown'
