...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'temp_product_media' not in context.user_data:
        context.user_data['temp_product_media'] = []
        context.user_data['last_instruction_message_id'] = None
    ...
    if update.message.photo:
        context.user_data['temp_product_media'].append({
            ...
            'media_type': 'photo',
            'timestamp': len(context.user_data['temp_product_media'])  # Utiliser la longueur comme index
        })
    ...
    # Envoyer le nouveau message d'instruction et stocker son ID
...
async def finish_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Trier les médias par leur timestamp avant de les sauvegarder
    if 'temp_product_media' in context.user_data:
        sorted_media = sorted(context.user_data['temp_product_media'], 
                            key=lambda x: x['timestamp'])
        # Nettoyer le timestamp avant de sauvegarder
        for media in sorted_media:
            del media['timestamp']
        context.user_data['temp_product_media'] = sorted_media
    ...
    # Supprimer le dernier message d'instruction s'il existe
    if context.user_data.get('last_instruction_message_id'):
        try:
            await context.bot.delete_message(
                chat_id=update.effective_chat.id,
