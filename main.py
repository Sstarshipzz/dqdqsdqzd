...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif 'media_count' not in context.user_data:  # Ajouter cette condition
        context.user_data['media_count'] = len(context.user_data['temp_product_media'])
    ...
    # Incrémenter le compteur pour maintenir l'ordre
    context.user_data['media_count'] += 1
    current_index = context.user_data['media_count']

    # Créer un nouveau média avec un index ordonné
    new_media = {
        'media_id': None,
        'media_type': None,
    ...
    # Trier la liste par order_index
    context.user_data['temp_product_media'].sort(key=lambda x: x.get('order_index', 0))
