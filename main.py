...
# Ajoutez cette nouvelle fonction pour gérer la finalisation des médias
...
async def handle_normal_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ajoutez ces gestionnaires pour la navigation entre les médias
        ...
        try:
            ...
            await query.answer()  # Répondre immédiatement au callback
            ...
            if product and 'media' in product:
                ...
                if direction == "next":
                    current_index = min(current_index + 1, len(product['media']) - 1)
                else:  # prev
                ...
                caption += f"\n\n📸 Image {current_index + 1}/{len(product['media'])}"
                ...
                if current_index < len(product['media']) - 1:
                    nav_buttons.append(
                        InlineKeyboardButton("➡️ Suivant", callback_data=f"next_media_{category}_{product_name}")
                    )
                ...
                try:
                    # Supprimer le message précédent si existe
                    last_message_id = context.user_data.get('last_product_message_id')
                    if last_message_id:
                        try:
                            await context.bot.delete_message(
                                chat_id=query.message.chat_id,
                    ...
                    # Stocker l'ID du nouveau message
                    context.user_data['last_product_message_id'] = message.message_id
            ...
            await query.answer("Une erreur est survenue lors du chargement du média")
