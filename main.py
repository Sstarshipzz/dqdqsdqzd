...
async def handle_product_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Stocker le message initial pour pouvoir le supprimer plus tard
    ...
    context.initial_media_message = message  # Stocker la référence du message
...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'temp_product_media' not in context.user_data:
        ...
        context.user_data['photos_count'] = 0
        context.user_data['videos_count'] = 0
    
    # Supprimer le message initial si c'est le premier média
    if len(context.user_data['temp_product_media']) == 0 and hasattr(context, 'initial_media_message'):
        try:
            await context.initial_media_message.delete()
            delattr(context, 'initial_media_message')
        except Exception as e:
            print(f"Erreur lors de la suppression du message initial: {e}")

    # Supprimer le message précédent d'instruction s'il existe
    if context.user_data.get('last_instruction_message_id'):
        try:
            await context.bot.delete_message(
                chat_id=update.effective_chat.id,
                message_id=context.user_data['last_instruction_message_id']
            )
        except Exception as e:
            print(f"Erreur lors de la suppression du message: {e}")

    # Créer un nouveau média avec priorité (photos d'abord, puis vidéos)
    if update.message.photo:
        context.user_data['photos_count'] += 1
        new_media = {
            'media_id': update.message.photo[-1].file_id,
            'media_type': 'photo',
            'order_index': context.user_data['photos_count'],
            'is_photo': True
        }
    elif update.message.video:
        context.user_data['videos_count'] += 1
        new_media = {
            'media_id': update.message.video.file_id,
            'media_type': 'video',
            'order_index': context.user_data['videos_count'],
            'is_photo': False
        }

    context.user_data['temp_product_media'].append(new_media)

    # Trier la liste : photos d'abord (par ordre), puis vidéos (par ordre)
    context.user_data['temp_product_media'].sort(key=lambda x: (
        not x.get('is_photo', True),  # False (vidéos) vient après True (photos)
        x.get('order_index', 0)
    ))
    ...
    # Envoyer le nouveau message d'instruction
    message = await update.message.reply_text(
        f"📸 {len(context.user_data['temp_product_media'])} média(s) ajouté(s)\n"
        "Continuez à envoyer des médias ou cliquez sur Terminé :",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Terminé", callback_data="finish_media")],
            [InlineKeyboardButton("🔙 Annuler", callback_data="cancel_add_product")]
        ])
    )
    context.user_data['last_instruction_message_id'] = message.message_id

    return WAITING_PRODUCT_MEDIA

async def finish_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Finalise l'ajout des médias et crée le produit"""
    ...
    # Créer le nouveau produit avec les médias dans l'ordre de leur ajout
    media_list = context.user_data.get('temp_product_media', [])
...
async def handle_normal_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Boutons d'annulation
        ...
        try:
            ...
            context.user_data['current_media_index'] = 0  # Réinitialiser l'index
            ...
            if product:
                ...
                if 'media' in product and product['media']:
                    ...
                    caption += f"\n\n📸 Média 1/{total_media}"  # Numérotation cohérente
